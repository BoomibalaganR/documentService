    ...
    body: Joi.object({
        ...
        documents: Joi.array().items(
            Joi.object({
                ...
                doc_type: Joi.string().valid('identity', 'personal').required().messages({
                    'string.base': 'doc_type should be a type of text',
                    'string.empty': 'doc_type cannot be an empty field',
                    'any.required': 'doc_type is a required field',
                    'any.only': 'doc_type must be one of [identity, personal]'
                })
            })
        ).required().messages({
            'array.base': 'documents should be an array',
            'array.includes': 'documents array must include valid items',
